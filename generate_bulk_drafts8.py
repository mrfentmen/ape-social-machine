#!/usr/bin/env python3
"""Batch 8 generator: Axel "Blitz" Beaumont character-sheet voice, batch 2.

Same rules as batch 7 (see generate_bulk_drafts7.py and BRAND-VOICE.md
'BLITZ CHARACTER SHEET VOICE'): B7 pools, 5-tag rule, full guardrails,
cross-batch dedupe including drafts_bulk7 + all stagings/queues.
...
OUT_JSON = os.path.join(HERE, "drafts_bulk8.json")
OUT_MD = os.path.join(HERE, "drafts_bulk8_preview.md")
...
def load_used_texts():
    ...
    files = ["drafts_bulk.json", "drafts_bulk2.json", "drafts_bulk3.json",
             ...
             "drafts_bulk7.json", "drafts_x_linkedin.json"]
    for fn in files:
...
def gen_platform(platform, used):
    ...
    r = random.Random(f"bulk8-{platform}")
    out, attempts = [], 0
    while len(out) < count and attempts < 2_000_000:
        ...
        if k in used:
            continue
        rng = random.Random(f"tags-bulk8-{platform}-{attempts}")
...
def main():
    ...
    result = {"note": ("Batch 8 Blitz drafts (character-sheet voice, batch 2). "
                       "Drafts only, nothing scheduled. Generated 2026-09-19.")}
    all_ok = True
    ...
    with open(OUT_MD, "w") as f:
        f.write("# Batch 8 drafts (Blitz character-sheet voice) - PREVIEW ONLY, nothing scheduled\n\n")
